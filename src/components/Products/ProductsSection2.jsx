// Auto-generated from products.html section 2
const ProductsSection2 = () => (
  <section className="products-section ptb-100">
  <div className="container">
  <div className="product-result">
  <div className="row align-items-center">
  <div className="col-lg-4 col-md-4">
  <p>Showing <span>09</span> results</p>
  </div>
  <div className="col-lg-8 col-md-8">
  <div className="row">
  <div className="col-lg-6">
  <form>
  <div className="form-floating">
  <input type="text" className="form-control" id="searchHere" placeholder="Search Here" />
  <label htmlFor="searchHere" className="form-label">Search Here</label>
  </div>
  </form>
  </div>
  <div className="col-lg-6">
  <select className="form-select form-control" aria-label="Default select example">
  <option selected="">Default Sorting</option>
  <option value="1">Bp Machine</option>
  <option value="2">Hand Sanitizer</option>
  <option value="3">Medicine</option>
  </select>
  </div>
  </div>
  </div>
  </div>
  </div>
  <div className="row justify-content-center">
  <div className="col-lg-4 col-sm-6">
  <div className="main-product-item">
  <div className="product-img">
  <a href="product-details.html">
  <img src="assets/images/products/product-1.jpg" alt="Image" />
  </a>
  <ul>
  <li>
  <a href="wishlist.html">
  <i className="icofont-heart-alt"></i>
  </a>
  </li>
  <li>
  <a href="#">
  <i className="icofont-eye-alt"></i>
  </a>
  </li>
  </ul>
  <a href="cart.html" className="main-btn">
  <span className="btn-style">Add To Cart</span>
  </a>
  </div>
  <div className="main-product-content d-flex">
  <div>
  <span className="tag">BEARING</span>
  <h3>
  <a href="product-details.html">
  											Wheel Bearing Retainer
  										</a>
  </h3>
  <span className="price"><del>500.00</del> $450.00</span>
  </div>
  <ul className="ms-auto">
  <li>
  <i className="icofont-star"></i>
  </li>
  <li>
  <i className="icofont-star"></i>
  </li>
  <li>
  <i className="icofont-star"></i>
  </li>
  <li>
  <i className="icofont-star"></i>
  </li>
  <li>
  <i className="icofont-star"></i>
  </li>
  </ul>
  </div>
  </div>
  </div>
  <div className="col-lg-4 col-sm-6">
  <div className="main-product-item">
  <div className="product-img">
  <a href="product-details.html">
  <img src="assets/images/products/product-2.jpg" alt="Image" />
  </a>
  <ul>
  <li>
  <a href="wishlist.html">
  <i className="icofont-heart-alt"></i>
  </a>
  </li>
  <li>
  <a href="#">
  <i className="icofont-eye-alt"></i>
  </a>
  </li>
  </ul>
  <a href="cart.html" className="main-btn">
  <span className="btn-style">Add To Cart</span>
  </a>
  </div>
  <div className="main-product-content d-flex">
  <div>
  <span className="tag">BRAKE</span>
  <h3>
  <a href="product-details.html">
  											Brake Conversion Kit
  										</a>
  </h3>
  <span className="price">$500.00</span>
  </div>
  <ul className="ms-auto">
  <li>
  <i className="icofont-star"></i>
  </li>
  <li>
  <i className="icofont-star"></i>
  </li>
  <li>
  <i className="icofont-star"></i>
  </li>
  <li>
  <i className="icofont-star"></i>
  </li>
  <li>
  <i className="icofont-star"></i>
  </li>
  </ul>
  </div>
  </div>
  </div>
  <div className="col-lg-4 col-sm-6">
  <div className="main-product-item">
  <div className="product-img">
  <a href="product-details.html">
  <img src="assets/images/products/product-3.jpg" alt="Image" />
  </a>
  <ul>
  <li>
  <a href="wishlist.html">
  <i className="icofont-heart-alt"></i>
  </a>
  </li>
  <li>
  <a href="#">
  <i className="icofont-eye-alt"></i>
  </a>
  </li>
  </ul>
  <a href="cart.html" className="main-btn">
  <span className="btn-style">Add To Cart</span>
  </a>
  </div>
  <div className="main-product-content d-flex">
  <div>
  <span className="tag">MOUNT</span>
  <h3>
  <a href="product-details.html">
  											Shock Mount Insulator
  										</a>
  </h3>
  <span className="price"><del>150.00</del> $100.00</span>
  </div>
  <ul className="ms-auto">
  <li>
  <i className="icofont-star"></i>
  </li>
  <li>
  <i className="icofont-star"></i>
  </li>
  <li>
  <i className="icofont-star"></i>
  </li>
  <li>
  <i className="icofont-star"></i>
  </li>
  <li>
  <i className="icofont-star"></i>
  </li>
  </ul>
  </div>
  </div>
  </div>
  <div className="col-lg-4 col-sm-6">
  <div className="main-product-item">
  <div className="product-img">
  <a href="product-details.html">
  <img src="assets/images/products/product-4.jpg" alt="Image" />
  </a>
  <ul>
  <li>
  <a href="wishlist.html">
  <i className="icofont-heart-alt"></i>
  </a>
  </li>
  <li>
  <a href="#">
  <i className="icofont-eye-alt"></i>
  </a>
  </li>
  </ul>
  <a href="cart.html" className="main-btn">
  <span className="btn-style">Add To Cart</span>
  </a>
  </div>
  <div className="main-product-content d-flex">
  <div>
  <span className="tag">TAIL</span>
  <h3>
  <a href="product-details.html">
  											Tail Light Lens
  										</a>
  </h3>
  <span className="price"><del>500.00</del> $300.00</span>
  </div>
  <ul className="ms-auto">
  <li>
  <i className="icofont-star"></i>
  </li>
  <li>
  <i className="icofont-star"></i>
  </li>
  <li>
  <i className="icofont-star"></i>
  </li>
  <li>
  <i className="icofont-star"></i>
  </li>
  <li>
  <i className="icofont-star"></i>
  </li>
  </ul>
  </div>
  </div>
  </div>
  <div className="col-lg-4 col-sm-6">
  <div className="main-product-item">
  <div className="product-img">
  <a href="product-details.html">
  <img src="assets/images/products/product-5.jpg" alt="Image" />
  </a>
  <ul>
  <li>
  <a href="wishlist.html">
  <i className="icofont-heart-alt"></i>
  </a>
  </li>
  <li>
  <a href="#">
  <i className="icofont-eye-alt"></i>
  </a>
  </li>
  </ul>
  <a href="cart.html" className="main-btn">
  <span className="btn-style">Add To Cart</span>
  </a>
  </div>
  <div className="main-product-content d-flex">
  <div>
  <span className="tag">COIL</span>
  <h3>
  <a href="product-details.html">
  											Coil Spring Conversion Kit
  										</a>
  </h3>
  <span className="price">$40.00</span>
  </div>
  <ul className="ms-auto">
  <li>
  <i className="icofont-star"></i>
  </li>
  <li>
  <i className="icofont-star"></i>
  </li>
  <li>
  <i className="icofont-star"></i>
  </li>
  <li>
  <i className="icofont-star"></i>
  </li>
  <li>
  <i className="icofont-star"></i>
  </li>
  </ul>
  </div>
  </div>
  </div>
  <div className="col-lg-4 col-sm-6">
  <div className="main-product-item">
  <div className="product-img">
  <a href="product-details.html">
  <img src="assets/images/products/product-6.jpg" alt="Image" />
  </a>
  <ul>
  <li>
  <a href="wishlist.html">
  <i className="icofont-heart-alt"></i>
  </a>
  </li>
  <li>
  <a href="#">
  <i className="icofont-eye-alt"></i>
  </a>
  </li>
  </ul>
  <a href="cart.html" className="main-btn">
  <span className="btn-style">Add To Cart</span>
  </a>
  </div>
  <div className="main-product-content d-flex">
  <div>
  <span className="tag">REPLICA</span>
  <h3>
  <a href="product-details.html">
  											OE Replica Wheels
  										</a>
  </h3>
  <span className="price"><del>700.00</del> $650.00</span>
  </div>
  <ul className="ms-auto">
  <li>
  <i className="icofont-star"></i>
  </li>
  <li>
  <i className="icofont-star"></i>
  </li>
  <li>
  <i className="icofont-star"></i>
  </li>
  <li>
  <i className="icofont-star"></i>
  </li>
  <li>
  <i className="icofont-star"></i>
  </li>
  </ul>
  </div>
  </div>
  </div>
  <div className="col-lg-4 col-sm-6">
  <div className="main-product-item">
  <div className="product-img">
  <a href="product-details.html">
  <img src="assets/images/products/product-7.jpg" alt="Image" />
  </a>
  <ul>
  <li>
  <a href="wishlist.html">
  <i className="icofont-heart-alt"></i>
  </a>
  </li>
  <li>
  <a href="#">
  <i className="icofont-eye-alt"></i>
  </a>
  </li>
  </ul>
  <a href="cart.html" className="main-btn">
  <span className="btn-style">Add To Cart</span>
  </a>
  </div>
  <div className="main-product-content d-flex">
  <div>
  <span className="tag">WHEEL</span>
  <h3>
  <a href="product-details.html">
  											Wheel Bearing Retainer
  										</a>
  </h3>
  <span className="price">$50,000.00</span>
  </div>
  <ul className="ms-auto">
  <li>
  <i className="icofont-star"></i>
  </li>
  <li>
  <i className="icofont-star"></i>
  </li>
  <li>
  <i className="icofont-star"></i>
  </li>
  <li>
  <i className="icofont-star"></i>
  </li>
  <li>
  <i className="icofont-star"></i>
  </li>
  </ul>
  </div>
  </div>
  </div>
  <div className="col-lg-4 col-sm-6">
  <div className="main-product-item">
  <div className="product-img">
  <a href="product-details.html">
  <img src="assets/images/products/product-8.jpg" alt="Image" />
  </a>
  <ul>
  <li>
  <a href="wishlist.html">
  <i className="icofont-heart-alt"></i>
  </a>
  </li>
  <li>
  <a href="#">
  <i className="icofont-eye-alt"></i>
  </a>
  </li>
  </ul>
  <a href="cart.html" className="main-btn">
  <span className="btn-style">Add To Cart</span>
  </a>
  </div>
  <div className="main-product-content d-flex">
  <div>
  <span className="tag">BRAKE</span>
  <h3>
  <a href="product-details.html">
  											Brake Conversion Kit
  										</a>
  </h3>
  <span className="price">$35.00</span>
  </div>
  <ul className="ms-auto">
  <li>
  <i className="icofont-star"></i>
  </li>
  <li>
  <i className="icofont-star"></i>
  </li>
  <li>
  <i className="icofont-star"></i>
  </li>
  <li>
  <i className="icofont-star"></i>
  </li>
  <li>
  <i className="icofont-star"></i>
  </li>
  </ul>
  </div>
  </div>
  </div>
  <div className="col-lg-4 col-sm-6">
  <div className="main-product-item">
  <div className="product-img">
  <a href="product-details.html">
  <img src="assets/images/products/product-9.jpg" alt="Image" />
  </a>
  <ul>
  <li>
  <a href="wishlist.html">
  <i className="icofont-heart-alt"></i>
  </a>
  </li>
  <li>
  <a href="#">
  <i className="icofont-eye-alt"></i>
  </a>
  </li>
  </ul>
  <a href="cart.html" className="main-btn">
  <span className="btn-style">Add To Cart</span>
  </a>
  </div>
  <div className="main-product-content d-flex">
  <div>
  <span className="tag">SHOCK</span>
  <h3>
  <a href="product-details.html">
  											Shock Mount Insulator
  										</a>
  </h3>
  <span className="price">$250.00</span>
  </div>
  <ul className="ms-auto">
  <li>
  <i className="icofont-star"></i>
  </li>
  <li>
  <i className="icofont-star"></i>
  </li>
  <li>
  <i className="icofont-star"></i>
  </li>
  <li>
  <i className="icofont-star"></i>
  </li>
  <li>
  <i className="icofont-star"></i>
  </li>
  </ul>
  </div>
  </div>
  </div>
  <nav>
  <ul className="pagination">
  <li className="page-item">
  <a href="#" className="page-link">
  <i className="icofont-simple-left"></i>
  </a>
  </li>
  <li className="page-item">
  <a href="#" className="page-link" aria-current="page">1</a>
  </li>
  <li className="page-item">
  <a href="#" className="page-link active">2</a>
  </li>
  <li className="page-item">
  <a href="#" className="page-link">3</a>
  </li>
  <li className="page-item">
  <a href="#" className="page-link">4</a>
  </li>
  <li className="page-item">
  <a href="#" className="page-link">
  <i className="icofont-simple-right"></i>
  </a>
  </li>
  </ul>
  </nav>
  </div>
  </div>
  </section>
);

export default ProductsSection2;
